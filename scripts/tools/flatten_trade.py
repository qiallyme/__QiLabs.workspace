#!/usr/bin/env python3
"""
Flatten Trade Folder - Move all files from subfolders to root of .trade/
"""

import shutil
from pathlib import Path

TRADE_DIR = Path(r"C:\Users\codyr\My Drive\.private\.trade")
...
def flatten_directory(root_dir: Path):
    ...
    for subdir in root_dir.iterdir():
        if subdir.is_dir():
            ...
            all_files = [f for f in files_in_subdir if f.is_file()]
            ...
            for file_path in all_files:
                ...
                while target_file.exists() and target_file != file_path:
                    # Add subdir name and counter to filename to avoid conflicts
                    stem = file_path.stem
                    suffix = file_path.suffix
                    target_file = root_dir / f"{subdir.name}_{stem}_{counter}{suffix}"
                    counter += 1
                ...
                try:
                    ...
                    if target_file != original_target:
                        print(f"  Moved: {file_path.name} -> {target_file.name} (renamed to avoid conflict)")
                    else:
